...
async function init(initv=1) {
    ...
    if(initv==1 || initv==2){           const response = await fetch('https://'+basedomain+'/create-connection');
...
if (ws.readyState === WebSocket.OPEN) {
    if(hbs<maxhb){hbs+=1
        ...
        console.log("[HT-HB]Heartbeat sent");}else{
...
function displayMessage(message, type) {
    if(message.type!="h"){const div = document.createElement('div');
